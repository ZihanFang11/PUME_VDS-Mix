...
def compute_all_scores(args,test_truth_label,softmax_scores):
    fpr_values = [0.005, 0.01, 0.05, 0.1, 0.5, 1]
    softmax_ccr, softmax_fpr,softmax_ccrs =  EvaluationMetrics.ccr_at_fpr(test_truth_label, softmax_scores)
    output_lines = ['CCR@FPR of {}% = {}'.format(fpr * 100, ccr) for fpr, ccr in zip(fpr_values, softmax_ccrs)]
    output = ', '.join(output_lines)
    with open(args.save_file, "a") as f:
        f.write('openness:{},unseen_num:{},dataset:{}\n'.format(args.openness, args.unseen_num, args.data))
        f.write(output)
        f.write("\n")
...
class EvaluationMetrics:
    ...
    @staticmethod
    def ccr_at_fpr(gt, scores, fpr_values=None, unk_label=-100):
        """
